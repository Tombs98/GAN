...
def generator(z):  # 生成器，z的维度为[N, 100]
    ...
    return G_log_prob, G_prob # 返回G_prob
...
def discriminator(x):  # 判别器，x的维度为[N, 784]
    ...
    return D_logit, D_prob  # 返回D_prob, D_logit
...
G_logit, G_sample = generator(Z)  # 取得生成器的生成结果
D_logit_real, D_real = discriminator(X)  # 取得判别器判别的真实手写数字的结果
D_logit_fake, D_fake = discriminator(G_sample)  # 取得判别器判别的生成的手写数字的结果
smooth = 0.1
D_loss_real = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=D_logit_real, labels=tf.ones_like(
    D_logit_real))*(1-smooth))  # 对判别器对真实样本的判别结果计算误差(将结果与1比较)
...
G_loss = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=D_logit_fake, labels=tf.ones_like(
    D_logit_fake))*(1-smooth))  # 生成器的误差(将判别器返回的对虚假样本的判别结果与1比较)
...
D_solver = tf.train.AdamOptimizer(0.0002).minimize(D_loss, var_list=theta_D)  # 判别器的训练器
G_solver = tf.train.AdamOptimizer(0.0002).minimize(G_loss, var_list=theta_G)  # 生成器的训练器

mb_size = 64  # 训练的batch_size
